...
class MakeShrinkMap(object):
    ...
    def __init__(
        self, min_text_size=8, shrink_ratio=0.4, keep_thin_text=True, **kwargs
    ):
        self.min_text_size = min_text_size
        self.keep_thin_text = keep_thin_text
    ...
    def __call__(self, data):
        ...
        for i in range(len(text_polys)):
            ...
            too_small = (
                ...
                and min(height, width) < self.min_text_size
            )
            if ignore_tags[i] or (too_small and not self.keep_thin_text):
                ...
                if shrunk == []:
                    if self.keep_thin_text:
                        cv2.fillPoly(
                            gt, [polygon.astype(np.int32)], 1
                        )
                    else:
                        cv2.fillPoly(
