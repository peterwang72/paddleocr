...
class DetLabelEncode(object):
    def __init__(self, ignore_tags_transcription=None, **kwargs):
        """
        ignore_tags_transcription: transcriptions treated as don't-care (no loss).
        Default ["*", "###"] matches PaddleOCR ICDAR convention.
        Set to [] to train/eval symbol boxes (underline, box, circle, etc.) as text regions.
        """
        if ignore_tags_transcription is None:
            self.ignore_tags_transcription = {"*", "###"}
        else:
            self.ignore_tags_transcription = set(ignore_tags_transcription)
    ...
    def __call__(self, data):
        ...
        for bno in range(0, nBox):
            ...
            boxes.append(box)
            txts.append(txt)
            txt_tags.append(txt in self.ignore_tags_transcription)
        if len(boxes) == 0:
            return None
