...
def train(
    ...
    batch_profile_log = config["Global"].get("batch_profile_log", None)
    batch_size_per_card = config["Train"]["loader"]["batch_size_per_card"]
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            if (global_step > 0 and global_step % print_batch_step == 0) or (
                ...
                if batch_profile_log and dist.get_rank() == 0:
                    log_dir = os.path.dirname(batch_profile_log)
                    if log_dir:
                    ...
                    write_header = not os.path.isfile(batch_profile_log)
                    with open(batch_profile_log, "a", newline="", encoding="utf-8-sig") as f:
                        writer = csv.writer(f)
                        if write_header:
                            writer.writerow(
                                [
                        ...
                        writer.writerow(
                            [
                                ...
                                global_step,
                                batch_size_per_card,
                                f"{train_reader_cost / print_batch_step:.5f}",
                                f"{train_batch_cost / print_batch_step:.5f}",
                                f"{total_samples / print_batch_step:.2f}",
                                f"{total_samples / train_batch_cost:.5f}"
                                if train_batch_cost > 0
                                else "0",
                                mem_reserved_mb,
                                mem_allocated_mb,
                            ]
