...
"""
PPLCNetV3 骨干网络（rec_lcnetv3.py）— 中文说明
============================================
PP-OCRv5 检测配置中的 Backbone: PPLCNetV3 即本文件中的类（det=True 时使用 NET_CONFIG_det）。
特点：多 stage 堆叠、可选 SE、支持检测/识别两套宽度与下采样策略；
识别分支常与 SVTR_LCNet 等 Head 组合。
============================================
"""
...
# =============================================================================
# PPLCNetV3（PP-LCNet V3）— PP-OCRv5_mobile_det.yml 中 Backbone.name 对应的实现
# -----------------------------------------------------------------------------
# NET_CONFIG_det：检测用各 stage 的卷积核、步长、是否 SE 等（与 det=True 分支配合）
# NET_CONFIG_rec：识别用配置（同一文件内复用 PPLCNetV3 类，scale/det 不同）
# =============================================================================
NET_CONFIG_det = {
    "blocks2":
...
class LCNetV3Block(nn.Layer):
    """LCNetV3 基本单元：可选 LearnableRep 多尺度 DW + PW；可选 SE；stride 可下采样。"""
...
class PPLCNetV3(nn.Layer):
    """
    PP-LCNet 第三代：检测(det=True)用 NET_CONFIG_det 与多尺度输出 outs；
    识别(det=False)用 NET_CONFIG_rec，forward 返回序列特征或池化向量视配置而定。
    scale 控制宽度；lr_mult_list 用于分层学习率。
    """
