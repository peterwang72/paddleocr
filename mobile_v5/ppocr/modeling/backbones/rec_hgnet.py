...
"""
HGNet 系列骨干与通用算子（rec_hgnet.py）— 中文说明
==================================================
本文件在 mobile_v5 快照中主要被 rec_lcnetv3 引用：MeanPool2D（固定窗口平均池化）。
完整 HGNet 实现亦在此文件中，用于部分识别/检测骨干配置。
==================================================
"""
import paddle
...
class MeanPool2D(nn.Layer):
    """将特征图自适应池化到固定 (h,w)，用于识别网络中序列下采样等。"""
...
class ConvBNAct(nn.Layer):
    """卷积 + BN + 可选 ReLU，padding 为 same 风格。"""
...
class ESEModule(nn.Layer):
    """Efficient SE 变体：1x1 压缩扩展后与输入逐元素相乘。"""
...
class HG_Block(nn.Layer):
    """
    HGNet 基本块：多层 3x3 串联，中间特征与输入 concat 后经 1x1 聚合 + ESE；
    identity=True 时输出与输入残差相加（需通道一致）。
    """
...
class HG_Stage(nn.Layer):
    """一个 HGNet stage：可选深度可分离式下采样，再堆叠多个 HG_Block。"""
...
class PPHGNet(nn.Layer):
    """
    Paddle 版 HGNet（PPHGNet）：多 stage 堆叠，常用于识别骨干（如 SVTR_HGNet 等配置）。

    Args:
        stem_channels: stem 各层通道配置
        stage_config: 每 stage 的通道数、步长等
        layer_num: 每个 HG_Block 内 3x3 层数
        use_last_conv: 分类头前是否再加 1x1
        class_expand: 末层 1x1 扩展通道
        dropout_prob: dropout 概率，0 表示不用
        class_num: 分类类别数
    """
