...
"""
PPLCNetV2 检测骨干（det_pp_lcnet_v2.py）— 中文说明
================================================
轻量级多阶段 CNN：stem + 4 个 stage，每层由 RepDepthwiseSeparable 堆叠；
可选重参数化（use_rep）将训练期多分支 depthwise 融合为单卷积以加速推理。
输出为多个 stage 的特征图列表 outs，供检测 Neck（如 DBFPN/RSEFPN）融合。

注意：PP-OCRv5_mobile_det 官方配置使用的是 PPLCNetV3（见 rec_lcnetv3.py），
本文件为另一套 PP-LCNetV2 实现，常用于对照或历史配置。
================================================
"""
...
# 各 stage 超参：输入通道基准、dw 核大小、是否拆分 pw、是否 rep、SE、shortcut
NET_CONFIG = {
...
class RepDepthwiseSeparable(nn.Layer):
    """
    深度可分离卷积块：DW（可选 rep 多核求和）→ 可选 SE → PW（可拆成两个 1x1）→ 可选残差。
    re_parameterize() 将训练期多分支 DW 融合为单一 dw_conv，便于部署。
    """
...
class PPLCNetV2(nn.Layer):
    """
    PP-LCNetV2 主体：scale 控制宽度；depths 为每 stage 重复块数；
    out_indx 指定哪些 stage 的输出加入 outs（多尺度给 FPN）。
    """
