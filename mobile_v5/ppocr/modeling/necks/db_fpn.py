...
"""
检测 Neck / FPN 模块（db_fpn.py）— 中文说明
==========================================
将骨干网络多尺度特征（通常 c2~c5）融合为单张特征图，供 DB 等检测头预测概率图。

- **DBFPN**：经典 DB 路径，1x1 对齐通道后自上而下相加，再 3x3 细化并上采样拼接；
  可选 **ASFBlock**（DBNet++ 自适应尺度融合）。
- **RSEFPN**：每层用 **RSELayer**（卷积 + SE + 残差），PP-OCRv5 mobile det 配置常用；
  可配合 **IntraCLBlock**（intracl=True）做层内跨位置交互。
- **LKPAN**：大核 PAN，mode 为 lite 时用 DSConv，否则用普通 Conv2D。
- **DSConv**：深度可分离风格的三段卷积块，供 LKPAN lite 等使用。
==========================================
"""
...
class DSConv(nn.Layer):
    """深度可分离式卷积栈：DW → 1x1 扩维 → 1x1 投影；通道变化时用 1x1 残差对齐。"""
...
class DBFPN(nn.Layer):
    """
    DB 系列常用的特征金字塔：四路输入统一到 out_channels，自顶向下融合后再
    分四路 3x3（通道为 out_channels//4）并上采样到同一分辨率后 concat。
    use_asf=True 时在 concat 后接 ASFBlock 做自适应加权融合。
    """
...
class RSELayer(nn.Layer):
    """Residual SE：卷积后 x + SE(x)（shortcut=False 时仅 SE(x)），增强通道表达。"""
...
class RSEFPN(nn.Layer):
    """
    带 SE 残差块的 FPN：先用 RSELayer(1x1) 对齐各尺度通道，再与 DBFPN 相同的上采样相加拓扑；
    后用 RSELayer(3x3) 得到四分路特征。intracl=True 时对 p2~p5 各接 IntraCLBlock。
    """
...
class LKPAN(nn.Layer):
    """Large Kernel PAN：多尺度融合路径；lite 模式用 DSConv 降低计算量。"""
...
class ASFBlock(nn.Layer):
    """
    DBNet++ 中的自适应尺度融合（ASF）。
    参考: https://github.com/MhLiao/DB/blob/master/decoders/feature_attention.py

    中文：对 concat 后的融合特征做卷积，再经空间/通道注意力对多尺度特征逐路加权后拼接。
    """

    def __init__(self, in_channels, inter_channels, out_features_num=4):
        """
        Args:
            in_channels: 输入通道数（融合特征）
            inter_channels: 中间层通道数
            out_features_num: 参与融合的尺度数量（与 features_list 长度一致）
        """
