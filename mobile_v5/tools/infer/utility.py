...
"""
推理公共工具（utility.py）— 中文说明
================================
供 PaddleOCR 各 `tools/infer/*.py` 共用，主要包括：

- **命令行**：`init_args` / `parse_args` —— 设备（GPU/XPU 等）、TensorRT、检测/识别模型路径、
  DB 二值化阈值与 unclip、识别字典与形状等。
- **预测引擎**：`create_predictor` —— 根据 `args.use_onnx` 走 ONNXRuntime，否则加载
  `model.pdiparams` + `model.json`（或 `model.pdmodel`），配置推理优化与张量句柄。
- **辅助**：`load_config`、`get_output_tensors`、`get_infer_gpuid`；以及可视化、旋转裁剪、
  大图切片与框合并等图像工具。

`mobile_v5` 内为与检测推理配套的引用快照；完整工程需安装/指向完整 `ppocr` 与权重目录。
================================
"""
import argparse
import os
...
def str2bool(v):
    """将命令行字符串转为布尔（兼容 true/yes/1 等写法）。"""
    return v.lower() in ("true", "yes", "t", "y", "1")


def str2int_tuple(v):
    """将形如 "3,48,320" 的字符串解析为整数元组。"""
    return tuple([int(i.strip()) for i in v.split(",")])


def init_args():
    """
    构建全局 ArgumentParser：涵盖推理设备、各任务模型路径、DB/EAST/SAST 等算法超参。
    检测相关常用项：`--det_model_dir`、`--det_limit_side_len`、`--det_db_*`。
    """
    ...
    # 文本框三分类（PP-LCNetV2，见 mobile_v5/configs/cls/pp_lcnet_v2_textbox_hw3.yml）
    parser.add_argument("--use_hw3_cls", type=str2bool, default=False)
    parser.add_argument("--hw3_cls_model_dir", type=str, default=None)
    parser.add_argument("--hw3_cls_image_shape", type=str, default="3, 48, 192")
    parser.add_argument(
        "--hw3_cls_label_list",
        type=str,
        default="printed,handwriting_text,handwriting_symbol",
        help="Comma-separated labels, index order must match training class ids",
    )
    parser.add_argument("--hw3_cls_batch_num", type=int, default=8)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If set, save DB detector internal probability maps/debug outputs into this directory.",
    )
...
def parse_args():
    """调用 init_args() 并解析命令行，返回 argparse.Namespace。"""
    parser = init_args()
...
def create_predictor(args, mode, logger):
    """
    创建推理会话。mode 决定读取 det_model_dir / rec_model_dir 等路径。

    - ONNX：`use_onnx=True` 时返回 (session, 输入描述, None, None)。
    - Paddle：返回 (predictor, input_tensor, output_tensors, config)；
      内部根据 GPU、TensorRT、MKLDNN、精度等设置 Config。
    """
    if mode == "det":
        ...
        model_dir = getattr(args, "hw3_cls_model_dir", None)
    elif mode == "rec":
...
def _convert_trt(
    ...
    """将 Paddle 静态图模型转为 TensorRT 引擎（需 paddle tensorrt 导出 API）。"""
...
def _pd_dtype_to_np_dtype(pd_dtype):
    """Paddle Inference 数据类型 → NumPy dtype（供 TRT 等路径使用）。"""
...
def load_config(file_path):
    """读取 YAML 配置文件（如 inference.yml），返回字典。"""
...
def get_output_tensors(args, mode, predictor):
    """
    从 predictor 取出输出 Tensor 句柄列表。
    识别 CRNN/SVTR 等若存在固定输出名 softmax_0.tmp_0 则只取该输出。
    """
...
def get_infer_gpuid():
    """
    根据环境变量 CUDA_VISIBLE_DEVICES（或 ROCm 下 HIP_VISIBLE_DEVICES）取第一个可见 GPU 编号。
...
def draw_ocr(
    ...
    """
    检测+识别结果可视化：先画框；若提供 txts 则在右侧拼接文字列表图。
    boxes 形状 (N, 4, 2)；scores 低于 drop_score 的框不绘制。
    """
    if scores is None:
        scores = [1] * len(boxes)
    box_num = len(boxes)
...
def base64_to_cv2(b64str):
    """Base64 字符串解码为 OpenCV BGR 图像（imdecode）。"""
    import base64
...
def draw_boxes(image, boxes, scores=None, drop_score=0.5):
    """按分数阈值过滤后绘制多边形检测框（与 draw_ocr 画框逻辑类似）。"""
    if scores is None:
        scores = [1] * len(boxes)
...
def get_rotate_crop_image(img, points):
    """
    将四点框透视变换为水平矩形裁剪图，供识别模型输入。
    points 为 4x2 顺时针/逆时针顺序；过高窄条会 rot90 转正。
    """
...
def get_minarea_rect_crop(img, points):
    """先求最小外接矩形，再排序四角点后调用 get_rotate_crop_image 裁剪。"""
...
def slice_generator(image, horizontal_stride, vertical_stride, maximum_slices=500):
    """
    大图滑窗切片生成器，产出 (子图, 垂直偏移, 水平偏移)。
    切片数超过 maximum_slices 时 assert 失败并提示增大 stride。
    """
...
def calculate_box_extents(box):
    """轴对齐包围盒范围（假定 box 为四点矩形，索引约定与 merge 逻辑一致）。"""
    min_x = box[0][0]
    max_x = box[1][0]
    min_y = box[0][1]
    max_y = box[2][1]
...
def merge_boxes(box1, box2, x_threshold, y_threshold):
    """若两框在 y 方向对齐且水平间隙小于阈值，则合并为更大矩形；否则返回 None。"""
...
def merge_fragmented(boxes, x_threshold=10, y_threshold=10):
    """
    合并同一行上被切碎的检测框（贪心合并）；若仍有可合并则递归调用直至稳定。
    """
    merged_boxes = []
    visited = set()
...
def check_gpu(use_gpu):
    """若请求 GPU 但未编译 CUDA 或当前设备为 CPU，则退回 CPU 推理。"""
    if use_gpu and (
